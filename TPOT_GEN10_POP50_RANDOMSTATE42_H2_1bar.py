...
tpot_data = pd.read_excel('H2_1BAR.xlsx',  dtype=np.float64)
features = tpot_data.drop('H2 Uptake 1 bar (mol/kg)', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['H2 Uptake 1 bar (mol/kg)'], train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -1.3379253727750501e-07
...
scale_min = -0.024
scale_max = 0.3  # Ensure the same range for both axes
...
plt.text(0.02, 0.98, '(e) 1 bar ', transform=g.ax_joint.transAxes, fontsize=30, verticalalignment='top')
