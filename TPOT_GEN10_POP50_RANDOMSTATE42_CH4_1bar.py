...
tpot_data = pd.read_excel('CH4_1BAR.xlsx',  dtype=np.float64)
features = tpot_data.drop('CH4 Uptake 1 bar (mol/kg)', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['CH4 Uptake 1 bar (mol/kg)'], train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -0.018900561256674168
exported_pipeline = make_pipeline(
    StackingEstimator(estimator=ExtraTreesRegressor(bootstrap=False, max_features=0.7500000000000001, min_samples_leaf=3, min_samples_split=12, n_estimators=100)),
...
scale_min = -0.1
scale_max = 5.5  # Ensure the same range for both axes
...
plt.text(0.02, 0.98, '(b) 1 bar ', transform=g.ax_joint.transAxes, fontsize=30, verticalalignment='top')
