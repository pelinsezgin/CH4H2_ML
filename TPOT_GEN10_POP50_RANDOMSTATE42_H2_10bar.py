...
tpot_data = pd.read_excel('H2_10BAR.xlsx',  dtype=np.float64)
features = tpot_data.drop('H2 Uptake 10 bar (mol/kg)', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['H2 Uptake 10 bar (mol/kg)'],  train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -3.498579056627762e-05
...
scale_min = -0.023
scale_max = 3.01  # Ensure the same range for both axes
...
plt.text(0.02, 0.98, '(f) 10 bar ', transform=g.ax_joint.transAxes, fontsize=30, verticalalignment='top')
