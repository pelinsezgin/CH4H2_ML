...
features = tpot_data.drop('CH4 Uptake 0.1 bar (mol/kg)', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['CH4 Uptake 0.1 bar (mol/kg)'], train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -0.0008822729437694325
exported_pipeline = make_pipeline(
    make_union(
        FunctionTransformer(copy),
        FunctionTransformer(copy)
    ),
    XGBRegressor(colsample_bytree=0.7, gamma=0, learning_rate=0.2, max_depth=9, min_child_weight=1, n_estimators=100, nthread=-1, reg_alpha=0.1, reg_lambda=0, subsample=0.6, verbosity=0)
)
...
scale_min = -0.1
scale_max = 3  # Ensure the same range for both axes
...
plt.text(0.02, 0.98, '(a) 0.1 bar ', transform=g.ax_joint.transAxes, fontsize=30, verticalalignment='top')
