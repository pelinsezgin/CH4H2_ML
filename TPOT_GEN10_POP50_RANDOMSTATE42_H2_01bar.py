...
tpot_data = pd.read_excel('H2_01BAR.xlsx', dtype=np.float64)
features = tpot_data.drop('H2 Uptake 0.1 bar (mol/kg)', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['H2 Uptake 0.1 bar (mol/kg)'], train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -8.77249945432786e-09
...
g.set_axis_labels(r'Simulated H$_{2}$ Uptake (mol/kg)', r'Predicted H$_{2}$ Uptake (mol/kg)', fontsize=30)

# Set both axes to the same scale
scale_min = -0.001
scale_max = 0.035  # Ensure the same range for both axes
...
plt.text(0.02, 0.98, '(d) 0.1 bar ', transform=g.ax_joint.transAxes, fontsize=30, verticalalignment='top')
...
plt.text(0.49, 0.16, f'R²: {r2_score(testing_target, preds):.2f}', fontsize=30, color='darkorange', transform=g.ax_joint.transAxes)
plt.text(0.01, 0.78, f'RMSE: {rmse_train:.4f}', fontsize=30, color='green', transform=g.ax_joint.transAxes)
plt.text(0.49, 0.09, f'RMSE: {rmse_test:.4f}', fontsize=30, color='darkorange', transform=g.ax_joint.transAxes)
plt.text(0.01, 0.71, f'MAE: {mean_absolute_error(training_target, y_pred_train):.4f}', fontsize=30, color='green', transform=g.ax_joint.transAxes)
plt.text(0.49, 0.02, f'MAE: {mean_absolute_error(testing_target, preds):.4f}', fontsize=30, color='darkorange', transform=g.ax_joint.transAxes)
